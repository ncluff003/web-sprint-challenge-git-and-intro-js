...
addArtist(
  ...
  `American`,
  `Lorem ipsum dolor sit amet consectetur adipisicing elit. Beatae odit, odio doloremque reprehenderit commodi hic omnis? Voluptates officiis, quam quisquam cumque esse tenetur, optio eum consequuntur ipsa natus nisi saepe.`
);
...
function lotsOfArt(artists) {
  ...
  for (let i = 0; i < artists.length; i++) {
    if (artists[i].paintings > 100) busyArtists.push(artists[i].name);
  }
  return busyArtists;
}
...
function artistByCountry(artists, nationality) {
  ...
  for (let i = 0; i < artists.length; i++) {
    if (artists[i].nationality === nationality) {
      countryArtists.push(artists[i].name);
    }
  }
